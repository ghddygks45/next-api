  ...
  return (
    <>
      ...
      <div className={styles.home}>
        <Image src={cutUrlImage} alt="가위로 주소 자르기" width={200} height={140} />
        <div className={styles.intro}>
          <h1 className={styles.title}>긴 주소를 짧은 주소로 줄이세요</h1>
          <p className={styles.description}>길고 복잡한 링크 주소를 짧게 줄이는 단축URL 서비스</p>
        </div>
        ...
        {shortUrl && (
          <form className={styles.form} onSubmit={handleCopy}>
            <Input className={`${styles.input} ${styles.shortUrl}`} readOnly value={`${process.env.NEXT_PUBLIC_BASE_URL}/${shortUrl}`} ref={inputRef} />
